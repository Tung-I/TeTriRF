...
if __name__=='__main__':
    ...
    for frameid in tqdm(range(0, args.numframe)):
        ...
        """
        keys(['xyz_min', 'xyz_max', 'viewfreq', 'density.grid', 'density.xyz_min', 'density.xyz_max', 
        ...
        if 'act_shift' in ckpt['model_state_dict']:
            # Q: How to interpret the act_shift?
            # A: The act_shift is used to adjust the activation of the density grid.
            # It is applied to the density values before computing the alpha values.
        ...
        for ind,plane in zip(['xy','xz','yz'],plane_data):
            ...
            print(f"Save plane image {ind} to {os.path.join(save_dir, f'{ind}_planes_frame_{frameid+1}.png')}")
    ...
    if not os.path.exists(save_dir):
        os.makedirs(save_dir, exist_ok=True)
    filename = f'{save_dir}/planes_to_videos.sh'
